...
<body>
    <div class="container">
        ...
        <p>Actuellement, chaque méthode de nos contrôleurs ont une ligne "include 'config.php'". 
            Par conséquent, config.php est rechargé beaucoup de fois (nombre de  contrôleurs * nombre de méthodes) et ce inutilement.
            Notre solution proposée consiste à stocker $root dans $_SESSION une seule fois dans le routeur. Ainsi, 
            tous les contrôleurs y accèderaient ensuite directement via $_SESSION['root'] sans recharger config.php.</p>
        ...
        <p>Cette solution présente plusieurs avantages. Tout d'abord, config.php n'est plus rechargé inutilement. 
            Ensuite, le code serait plus propre et cohérent avec l'utilisation des sessions.</p>
        ...
        <p>Il faudrait d'abord ajouter la ligne suivante dans le routeur : <br> $_SESSION['root'] = dirname(dirname(__DIR__)) . "/"; <br>
            Ensuite, dans chaque contrôleur, on remplace les include 'config.php' par <br> $root = $_SESSION['root']; <br> 
            Enfin, si on supprime l'include, nous n'avons plus accès à DEBUG donc il faudra le redéfinir dans le routeur : <br>
            if (!defined('DEBUG')) {
            define('DEBUG', FALSE);
            }
            <br>
